...
// ---------------------------------------------------------------------------
// Types — unit_price を含まない
// ---------------------------------------------------------------------------

type Company     = { id: string; name: string }
type Site        = { id: string; name: string }
type FieldProduct = { id: string; name: string }  // unit_price は受け取らない
  ...
  return (
    <div style={{ position: 'relative' }}>
      <input
        ...
        style={{ ...s.input, backgroundColor: disabled ? '#f3f4f6' : '#fff' }}
      />
...
// ---------------------------------------------------------------------------
// FieldDeliveryForm — 価格情報を一切受け取らない・表示しない
// ---------------------------------------------------------------------------

export default function FieldDeliveryForm({ initialCompanies, initialProducts }: Props) {
  ...
  const [products,  setProducts]  = useState<FieldProduct[]>(initialProducts)
  ...
  async function handleAddProduct(key: string) {
    ...
    const json = await res.json()
    if (!res.ok) { setError(json.error); return }
    // products API が返す product オブジェクトから id, name のみ取り出す
    const prod: FieldProduct = { id: json.product.id, name: json.product.name }
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      // /delivery 側の complete ページへリダイレクト
      router.push(`/delivery/${json.delivery.id}/complete`)
    } finally {
      setSubmitting(false)
    }
  }
    ...
    <main style={s.main}>
      ...
      <section style={s.card}>
        ...
        <div style={s.itemList}>
          {items.map((item, idx) => {
            const selectedProduct = toOptions(products).find(p => p.id === item.product_id)
            return (
  ...
  return (
    <div style={ia.wrap}>
      ...
      <div style={ia.row}>
        ...
        <button onClick={onCancel} style={ia.btnCancel}>キャンセル</button>
      </div>
    </div>
  )
}
...
const s: Record<string, CSSProperties> = {
  main: {
    maxWidth: 448, margin: '0 auto', padding: '16px 16px 120px',
    fontFamily: 'system-ui, -apple-system, sans-serif',
    backgroundColor: '#f9fafb', minHeight: '100dvh',
  },
  ...
  errorBox: {
    ...
    borderRadius: 8, padding: '10px 14px', fontSize: 14, margin: '0 0 12px',
  },
  card: {
  ...
  selectedValue: { fontSize: 16, fontWeight: 500, color: '#111827', padding: '6px 0', margin: 0 },
  changeBtn: {
    background: 'none', border: 'none', fontSize: 13, color: '#2563eb',
    cursor: 'pointer', padding: '4px 8px', minHeight: 44, flexShrink: 0,
  },
  input: {
    width: '100%', padding: '10px 12px', border: '1px solid #d1d5db',
    borderRadius: 8, fontSize: 16, boxSizing: 'border-box', minHeight: 44, color: '#111827',
  },
  hint: { fontSize: 14, color: '#9ca3af', margin: 0 },
  itemList: { display: 'flex', flexDirection: 'column', gap: 12 },
  itemBox: {
    border: '1px solid #e5e7eb', borderRadius: 8, padding: 12,
    backgroundColor: '#f9fafb', display: 'flex', flexDirection: 'column', gap: 8,
  },
  ...
  qtyLabel: { fontSize: 13, color: '#374151', whiteSpace: 'nowrap', flexShrink: 0 },
  qtyInput: {
    flex: 1, padding: '10px 12px', border: '1px solid #d1d5db',
    borderRadius: 8, fontSize: 16, minHeight: 44, boxSizing: 'border-box',
  },
  addItemBtn: {
    marginTop: 4, padding: '12px 0', width: '100%',
    background: '#fff', border: '1px dashed #d1d5db', borderRadius: 8,
    fontSize: 14, color: '#2563eb', cursor: 'pointer', minHeight: 44,
  },
  submitBtn: {
    position: 'fixed', bottom: 24, left: 16, right: 16, padding: '16px 0',
    background: '#2563eb', color: '#fff', border: 'none', borderRadius: 12,
    fontSize: 16, fontWeight: 700, cursor: 'pointer', minHeight: 56,
  },
}
...
const cb: Record<string, CSSProperties> = {
  list: {
    ...
    boxShadow: '0 4px 16px rgba(0,0,0,0.14)', listStyle: 'none', padding: 0, margin: '4px 0 0',
  },
  item: {
    padding: '12px 14px', fontSize: 15, color: '#111827', cursor: 'pointer',
    borderBottom: '1px solid #f3f4f6', minHeight: 44, display: 'flex', alignItems: 'center',
  },
  empty: { padding: '12px 14px', fontSize: 14, color: '#9ca3af', listStyle: 'none' },
  addNew: {
    padding: '12px 14px', fontSize: 14, fontWeight: 600, color: '#2563eb',
    cursor: 'pointer', borderTop: '1px solid #e5e7eb', minHeight: 44,
...
const ia: Record<string, CSSProperties> = {
  ...
  btnCancel: {
    flex: 1, padding: '10px 0', background: '#fff', color: '#6b7280',
