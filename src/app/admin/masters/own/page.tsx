...
export default function OwnMasterPage() {
  ...
  const profileField = (label: string, key: keyof Profile, placeholder = '') => (
    <div style={{ marginBottom: 16 }}>
      <label style={{ display: 'block', fontWeight: 600, marginBottom: 4, fontSize: 13, color: '#777777' }}>{label}</label>
      <input
        value={profile[key] ?? ''}
        onChange={e => setProfile(p => ({ ...p, [key]: e.target.value }))}
        placeholder={placeholder}
        style={{ width: '100%', padding: '8px 10px', border: '1px solid #D0CAC3', borderRadius: 6, fontSize: 14, boxSizing: 'border-box', background: '#FFFFFF', color: '#333333' }}
      />
    </div>
  )
  ...
  if (loadingProfile) return <p style={{ color: '#888888' }}>読み込み中…</p>

  return (
    <div style={{ fontFamily: 'system-ui, -apple-system, sans-serif' }}>
      {/* ── 自社設定 ──────────────────────────────────────────────── */}
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 24, flexWrap: 'wrap' }}>
        <h2 style={{ margin: 0, color: '#333333' }}>自社設定</h2>
      ...
      <form onSubmit={saveProfile} style={{ maxWidth: 560, marginBottom: 48 }}>
        ...
        <div style={{ marginBottom: 24 }}>
          <label style={{ display: 'block', fontWeight: 600, marginBottom: 8, fontSize: 13, color: '#777777' }}>
            会社ロゴ
            <span style={{ fontSize: 11, color: '#888888', marginLeft: 8, fontWeight: 400 }}>
              （最大2MB、PNG / JPEG / SVG）
            </span>
          </label>
          <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 12 }}>
            ...
            <label
              ...
              style={{
                padding: '8px 16px',
                background: uploadingLogo ? '#F0EDE8' : '#FFD700',
                color: uploadingLogo ? '#888888' : '#000',
                border: 'none',
                borderRadius: 6,
          ...
          <p style={{ fontSize: 12, color: '#777777', margin: '4px 0 8px', lineHeight: 1.5 }}>
            推奨サイズ：縦50px × 横200px以内
          </p>
          {msg && (
            <p style={{
              fontSize: 12,
              color: msg.includes('エラー') ? '#DC2626' : '#16A34A',
              marginTop: 8,
              textAlign: 'center',
            }}>
              {msg}
      ...
      <hr style={{ border: 'none', borderTop: '1px solid #E5E0DA', marginBottom: 32 }} />

      {/* ── 銀行口座 ──────────────────────────────────────────────── */}
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20 }}>
        <h2 style={{ margin: 0, color: '#333333' }}>銀行口座</h2>
        <button onClick={startNewBank} style={btnPrimary}>＋ 追加</button>
      </div>

      {bankEditing && (
        <div style={{ background: '#FFFFFF', border: '1px solid #E5E0DA', borderRadius: 8, padding: 20, marginBottom: 24, maxWidth: 480, boxShadow: '2px 2px 0 #E5E0DA' }}>
          <h3 style={{ marginTop: 0, marginBottom: 14, color: '#333333' }}>{bankEditing === 'new' ? '新規口座' : '口座を編集'}</h3>
          {bankInp('銀行名 *', 'bank_name')}
          {bankInp('支店名', 'branch_name')}
          <div style={{ marginBottom: 12 }}>
            <label style={{ display: 'block', fontSize: 12, fontWeight: 600, marginBottom: 3, color: '#777777' }}>口座種別</label>
            <select
          ...
          <div style={{ marginBottom: 14 }}>
            <label style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, cursor: 'pointer', color: '#555555' }}>
              <input
                type="checkbox"
      ...
      {loadingBank ? <p style={{ fontSize: 14, color: '#888888' }}>読み込み中…</p> : (
        <>
          <p style={{ fontSize: 13, color: '#888888', marginBottom: 8 }}>{accounts.length}件登録済み</p>
          <table style={{ width: '100%', borderCollapse: 'collapse', background: '#FFFFFF', border: '1px solid #E5E0DA', borderRadius: 8, overflow: 'hidden', maxWidth: 800, boxShadow: '2px 2px 0 #E5E0DA' }}>
            <thead>
            ...
            <tbody>
              {accounts.map(a => (
                <tr key={a.id} style={{ borderBottom: '1px solid #F0EDE8' }}>
                  <td style={{ padding: '10px 12px', fontSize: 14, color: '#555555' }}>
                    {a.bank_name}
                    {a.is_default && (
                      <span style={{ marginLeft: 6, fontSize: 11, padding: '2px 7px', borderRadius: 10, background: '#DCFCE7', color: '#16A34A', fontWeight: 600 }}>
                        デフォルト
                      </span>
                    )}
                  </td>
                  <td style={{ padding: '10px 12px', fontSize: 14, color: '#555555' }}>{a.branch_name ?? '—'}</td>
                  <td style={{ padding: '10px 12px', fontSize: 14, color: '#555555' }}>{a.account_type ?? '—'}</td>
                  <td style={{ padding: '10px 12px', fontSize: 14, color: '#555555' }}>{a.account_number ?? '—'}</td>
                  <td style={{ padding: '10px 12px', fontSize: 14, color: '#555555' }}>{a.account_holder ?? '—'}</td>
                  <td style={{ padding: '8px 12px', whiteSpace: 'nowrap' }}>
                    <button onClick={() => startEditBank(a)} style={btnSmall('#888888')}>編集</button>
                    {!a.is_default && (
              ...
              {accounts.length === 0 && (
                <tr><td colSpan={6} style={{ padding: 24, textAlign: 'center', color: '#888888' }}>口座が登録されていません</td></tr>
              )}
            </tbody>
          </table>
        </>
